...
`LazyEditor` is the startup boundary for BlockNote, KaTeX, editor extensions, and editor-only CSS. It keeps that chunk unloaded until an active note path exists. Startup milestone ownership lives in `startupPerformance.ts`: ordered frontend timings are relative to renderer module initialization, native elapsed time comes from `StartupTimingState`, and analytics payloads may contain timing, entry counts, source, and target status but never paths or note data. Setting `TOLARIA_STARTUP_TRACE=1` mirrors each milestone as structured JSON on stderr for native profiling.
