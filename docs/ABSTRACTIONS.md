...
The Quick Launcher renderer treats `(vaultId, relativePath)` as note identity and delegates cross-vault opening to the main window through the existing deep-link resolver. Its result list reuses `NoteSearchList`, presents native indexed-search titles directly, and derives workspace badges from the in-memory vault registry. It deliberately does not load vault entries on mount, so the idle launcher path remains constant-time with respect to registered vault contents. `show_quick_launcher` owns the native window lifecycle, including hidden-first construction, transparent native/window backing surfaces, singleton reuse, and reveal/focus ordering; `openQuickLauncherWindow` supplies its native title and error templates from the active locale catalog, preserving `{error}` for Rust-side interpolation. The critical `index.html` boot script owns the matching pre-React canvas state through `quick-launcher-native-window`; the renderer never compensates for an opaque WKWebView or startup shell with CSS. The native Tauri run loop owns focus-loss dismissal for the `quick-launcher` window so outside-click behavior does not depend on the renderer mounting or subscribing successfully; the renderer owns Escape dismissal. Every non-empty input remains available as a create-note action below the results, including when matches exist. Capture destinations use the pure `resolveQuickCaptureDestination` precedence rule (configured, active, last successful), but a configured unavailable vault is an attention state rather than permission to silently fall back. The launcher exposes only a bottom vault selector and always creates at the selected vault root. The input becomes the note H1 and filename stem; `create_quick_launcher_note` owns slugging, collision suffixes, vault-boundary validation, and the create-only write as one native operation so renderer lifecycle failures cannot be confused with failed persistence.
