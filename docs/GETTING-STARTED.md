...
The global Quick Launcher uses a native OS registration rather than the focused-window command registry. Its default is `Cmd/Ctrl+Shift+Space`; Settings records a replacement chord and reports registration conflicts while preserving the last working shortcut. The shortcut focuses one singleton `quick-launcher` webview even when the main window is hidden. Search spans opted-in registered vaults, while Capture saves with `Cmd/Ctrl+Enter` to the visible destination shown in the launcher.
